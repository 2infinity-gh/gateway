...
import { ApeswapConfig } from './apeswap.config';
import routerAbi from './ApeswapRouter.json';
import {
  Fetcher,
  Percent,
  Router,
...
export class Apeswap implements Uniswapish {
  private static _instances: { [name: string]: Apeswap };
  ...
  private constructor(network: string) {
    const config = ApeswapConfig.config;
    this.binancesmartchain = BinanceSmartChain.getInstance(network);
    this.chainId = this.binancesmartchain.chainId;
  ...
  public static getInstance(chain: string, network: string): Apeswap {
    if (Apeswap._instances === undefined) {
      Apeswap._instances = {};
    }
    if (!(chain + network in Apeswap._instances)) {
      Apeswap._instances[chain + network] = new Apeswap(network);
    }

    return Apeswap._instances[chain + network];
  }
  ...
  public getAllowedSlippage(allowedSlippageStr?: string): Percent {
    ...
    const allowedSlippage = ApeswapConfig.config.allowedSlippage;
   ...
   * @param apeswapRouter smart contract address
  ...
  async executeTrade(
    ...
    const contract = new Contract(apeswapRouter, abi, wallet);
